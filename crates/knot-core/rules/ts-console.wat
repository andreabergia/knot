(module
  ...
  (data (i32.const 16) "{\"abi_version\":1,\"id\":\"knot/ts-console\",\"name\":\"No console\",\"severity\":\"warning\"}")
  (data (i32.const 128) "\"kind\":\"call\",\"span\":")
  (data (i32.const 256) ",\"callee\":\"console.")
  (data (i32.const 512) "{\"rule_id\":\"knot/ts-console\",\"severity\":\"warning\",\"message\":\"Unexpected console statement.\",\"span\":")
  ...
  (func (export "knot_metadata") (result i64)
    (call $pack (i32.const 16) (i32.const 81)))
  ...
  (func (export "knot_check") (param $ptr i32) (param $len i32) (result i64)
    ...
    (block $done
      (loop $scan
        (br_if $done
          (i32.gt_u (i32.add (local.get $cursor) (i32.const 21)) (local.get $limit)))

        (if (call $matches_marker (local.get $cursor) (i32.const 128) (i32.const 21))
          (then
            (local.set $span_start (i32.add (local.get $cursor) (i32.const 21)))
            (local.set $span_end (i32.add (local.get $cursor) (i32.const 21)))
            ...
            (if (call $matches_marker
                  (i32.add (local.get $span_end) (i32.const 1))
                  (i32.const 256)
                  (i32.const 19))
              (then
                ...
                (call $copy (local.get $output_cursor) (i32.const 512) (i32.const 99))
                (local.set $output_cursor
                  (i32.add (local.get $output_cursor) (i32.const 99)))
                (call $copy
